...
public:
    ...
    void withdraw(double amt){
        ...
        else { balance-=amt; cout<<"Withdrawal successful. New balance: "<<balance<<endl; }
    }
...
public:
    void addAccount(int a,double b){
        for(auto &x:accounts)
            if(x.getAccNo()==a){ cout<<"Account number already exists!\n"; return; }
    ...
    Account* findAccount(int a){
        for(auto &x:accounts)
            if(x.getAccNo()==a) return &x;
        return nullptr;
    }

    void withdrawMoney(int a,double amt){
